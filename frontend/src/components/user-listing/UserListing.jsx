...
const UserListing = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const response = await axios.get('https://task-1-0ob5.onrender.com/users');
...
const UserCard = ({ user }) => {
  ...
  return (
    <div className="user-card">
      <img
        src={`https://task-1-0ob5.onrender.com/profile-image/${user._id}`}
      ...
      <div className="videos">
        {displayedVideos.map((video, index) => (
          <video key={index} controls width="200" height="200" className="video-item">
            <source src={`https://task-1-0ob5.onrender.com/video/${user._id}/${index}`} type={video.contentType} />
